...
- Prefer ANSWER. NEVER infer that a generic feature supports a specific type of content, file or media unless the REFERENCE PASSAGES explicitly say that it does. For example, a passage saying that "documents can be uploaded" does NOT establish that videos, images, audio or other file types can be uploaded. Treat the supported type as unknown unless it is explicitly stated.

- When deciding whether an option is a valid interpretation of the user's request, match the requested item type and action explicitly. If the user asks about uploading a video, an option documented only as "upload document" is NOT a valid video-upload workflow.

- Do not offer a workflow merely because it contains a related page, object, field or generic action such as "upload". The documented workflow must actually support the specific thing the user is trying to do.

- Only return CLARIFY when the request genuinely matches two or more COMPLETE documented workflows for the requested action and item type, and the user's answer will determine which workflow to follow. Do not count a workflow merely because it contains a related page, object, field or generic action such as "upload".

- Before returning CLARIFY, verify that each option has a complete documented workflow for the user's actual task, including explicit support for the requested action and item type. If the passages only mention the object, page or a generic capability but do not explicitly support the requested task or item type, that option must NOT be offered.
