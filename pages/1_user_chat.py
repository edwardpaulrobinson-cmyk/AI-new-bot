...
[data-testid="stMainBlockContainer"], .block-container { max-width:760px !important; margin:1.2rem auto 0 !important; min-height:calc(100vh - 6rem); background:#FFFFFF !important; border:1px solid #E1E7F0; border-bottom:none; border-radius:16px 16px 0 0; box-shadow:0 4px 24px rgba(16,24,40,.06); padding:1.3rem 1.6rem 6.5rem !important; }
...
section[data-testid="stBottom"], [data-testid="stBottom"] > div { background:#E6EBF2 !important; }
[data-testid="stBottomBlockContainer"] { max-width:760px !important; margin:0 auto; background:#FFFFFF !important; border:1px solid #E1E7F0; border-top:1px solid #EDF1F6; border-radius:0 0 16px 16px; box-shadow:0 6px 20px rgba(16,24,40,.06); padding:10px 16px 14px !important; }
[data-testid="stChatInput"] { background:#F4F7FB !important; border:1px solid #E1E7F0 !important; border-radius:14px !important; }
[data-testid="stChatInput"] textarea { color:#1A2230 !important; }
[data-testid="stChatInput"] textarea::placeholder { color:#8A94A3 !important; }
...
RELEVANCE_MIN = float(config.get_secret("RELEVANCE_MIN") or "0.35")
...
def triage_question(question, context_text, recent=""):
    ...
    sys_rules = (
        ...
        "   (b) it is ONE task, but the correct steps would MATERIALLY DIFFER depending on the "
        "user's situation or a prerequisite they haven't stated (e.g. for 'publish to portals', "
        "whether they have ALREADY added the property or are adding it new) — ask the single "
...
if prompt := st.chat_input("Ask a question (attach files above if needed)..."):
    ...
    try:
        kb_results = retriever.query(retrieval_query, k=8)
        retrieved = rag.format_context(kb_results)
    except Exception as e:
        safe_error(e, context="retrieval")
        kb_results, retrieved = [], ""
    try:
        rel = retriever.relevance(retrieval_query)
    except Exception:
        rel = 0.0
...
if pending and not st.session_state.get("resolved"):
    ...
    if st.session_state.get("show_escalation"):
        with st.form("callback"):
            st.markdown("No problem — I'll arrange for one of our account managers to give you a call. "
            ...
            st.caption("Fill in a time and click the button below.")
            sent = st.form_submit_button("Send callback request", type="primary")
            if sent:
                if not when.strip():
                    ...
                    if config.EMAIL_ENABLED:
                        # Give the account manager the FULL conversation for context, not just
                        # the last Q&A — skip the opening greeting (messages[0]).
                        transcript = "\n\n".join(
                        ...
                        body = (f"Callback request from the GNB Property assistant.\n\n"
                                ...
                                f"===== FULL CONVERSATION =====\n{transcript}\n===== END OF CONVERSATION =====\n")
                        send_escalation(subject=f"[Callback request] {pending['question'][:60]}",
                                        body=body, attachments=pending.get("assets"))
                    interaction_log.log_event(status="callback_requested", question=pending["question"],
                                              answer=pending["answer"], user=user_id, mode=mode,
                                              provider=pending.get("provider", ""),
