...
- Do not infer that a specific media type is supported by a workflow that is explicitly limited to a different type. For example, "documents can be uploaded" does NOT mean video files can be uploaded. However, a documented generic "file" or "attachment" upload workflow may be considered relevant to a file-upload question unless the documentation explicitly excludes that file type. Always follow any explicit file-type restrictions in the reference passages.

- Only return CLARIFY when the request genuinely matches two or more COMPLETE documented workflows for the requested action, and the user's answer will determine which workflow to follow. Do not count a workflow merely because it shares a page name, CRM object or generic word such as "upload". A workflow explicitly about documents is not a valid video-upload workflow, but a generic file or attachment upload workflow is relevant unless the documentation restricts the file types.
