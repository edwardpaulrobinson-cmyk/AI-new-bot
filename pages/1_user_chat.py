...
def triage_question(question, context_text, recent=""):
    ...
    sys_rules = (
        ...
        "overlaps with one (e.g. 'I can't get in touch with Robin' is NOT 'assign an enquiry to Robin' or "
        "'find a contact' — it may just be someone struggling to reach a colleague). If the specific "
        "intent isn't clear, CLARIFY by asking plainly what they're trying to do — do NOT present "
        "documented-procedure options as if you already know what they mean.\n"
...
SYSTEM_RULES = (
    ...
    "- Do NOT force a vague or general message onto a documented procedure just because a NAME or word overlaps with one. For example, 'I can't get in touch with Robin' is NOT necessarily 'assign an enquiry to Robin' or 'find a contact' — it may simply be someone struggling to reach a colleague. When you can't tell what specific CRM task (if any) they mean, ask plainly what they're trying to do; if they're just trying to reach a person, offer to arrange a callback. Never guess a procedure from a keyword and present its steps or options.\n"
