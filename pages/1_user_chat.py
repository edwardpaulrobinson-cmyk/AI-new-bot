...
- Match the user's requested ACTION and ITEM TYPE when deciding whether a workflow directly supports what they asked for. Do not treat different actions or item types as the same thing. For example, uploading a video file, adding a YouTube/TikTok video link, and uploading a document are different capabilities.

- NEVER infer that a generic or differently named capability supports a specific file or media type. For example, a workflow that explicitly says "upload document" must NOT be treated as support for uploading a video file. However, a generic "file" or "attachment" upload workflow may be considered relevant unless the documentation explicitly excludes that file type.

- If the exact method requested by the user is not documented, but the REFERENCE PASSAGES clearly describe a documented alternative that accomplishes the user's underlying goal, return ANSWER so the answer stage can explain the documented alternative. The answer must clearly distinguish the documented alternative from the method the user originally asked about. Do not claim the original method is supported.

- Only return CLARIFY when the request genuinely matches two or more COMPLETE documented workflows that could each accomplish the user's underlying goal. Do not count a workflow merely because it shares a page, object or generic word such as "upload". Before returning CLARIFY, reject options that are explicitly for a different content type or action. If there is only one documented relevant workflow or documented alternative, return ANSWER. If there is no relevant documented workflow or alternative, return NONE.
